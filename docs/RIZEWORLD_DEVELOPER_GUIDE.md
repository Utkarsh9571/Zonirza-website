# RIZEWORLD DEVELOPER GUIDE — ZONIRAZ PLATFORM ARCHITECTURE

Welcome to the Zoniraz developer guide. This document is a comprehensive, implementation-level technical architecture and onboarding guide for engineering teams maintaining and extending the Zoniraz platform.

---

## 1. Project Overview

Zoniraz is a luxury online jewelry storefront powered by a dynamic, database-driven pricing engine. Unlike typical e-commerce storefronts that display static prices, Zoniraz computes real-time pricing based on gold market rates, gemstone and diamond pricing grades, category making charge percentages, and size/length weight calculations.
...
* **Storage**: VPS Local Storage outside repository boundaries (`/opt/zoniraz-storage`)
...
flowchart TD
    ...
    subgraph Image Uploads
        ...
        SharpImage --> VPSMedia[(VPS Local Storage /opt/zoniraz-storage)]
    end
```
...
Below is an overview of the key folders within the Zoniraz repository:
...
Zoniraz models are mapped to the database using Mongoose. Here are the core specifications of the primary models:
...
Zoniraz isolates dynamic media assets from the deployment repository to optimize build speed and prevent workspace git bloat.

### VPS Storage Directory Structure
All user and admin uploads are saved in `/opt/zoniraz-storage` (fallback path on Windows systems: `C:\opt\zoniraz-storage`).
...
3. **Storage**: Writes optimized WebP files to `/opt/zoniraz-storage/public/images/products/[category]/`.
...
A critical requirement of the Zoniraz storefront is price consistency: a product card must never show a different price than the PDP initial state, the checkout cart, or the server-calculated order total.
...
* **Verify VPS Storage link**: Next.js requires a symbolic link or static router configured to serve images from outside the project root. On production servers, check that Nginx is configured to alias `/images/` paths to `/opt/zoniraz-storage/public/images/`.
...
```bash
mkdir -p /opt/zoniraz-storage/public/images/products
chown -R www-data:www-data /opt/zoniraz-storage
chmod -R 775 /opt/zoniraz-storage
```
...
STORAGE_ROOT=/opt/zoniraz-storage
