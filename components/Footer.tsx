      ...
      <div className="max-w-350 mx-auto px-6 md:px-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 lg:gap-8 mb-12 border-b border-[#EAE1D5]/10 pb-12">
          ...
          <div className="flex flex-col items-center md:items-start text-center md:text-left space-y-6">
            <LinkNext href="/" className="group flex items-center space-x-2">
              <span className="text-xl font-serif font-bold tracking-[0.15em] text-[#EAE1D5] group-hover:text-brand-gold transition-colors duration-500">
                LUXURY STARTER
              </span>
            </LinkNext>
            <p className="text-sm text-[#EAE1D5]/90 font-light tracking-wide max-w-75">
              {settings?.footerText || "Luxury Jewelry Commerce Starter is a premium, open-source luxury jewelry commerce store template. Designed with elegant aesthetics, formula-driven pricing, and customizable specifications for modern developers."}
            </p>
          </div>
          ...
          <div className="flex flex-col items-center md:items-start text-center md:text-left">
            ...
            <ul className="space-y-3 text-sm font-light text-[#EAE1D5]/70 flex flex-col items-center md:items-start">
              ...
              <li><LinkNext href="/about" className="hover:text-white transition-colors">About Us</LinkNext></li>
            </ul>
          </div>
            ...
            <div className="space-y-6 text-sm font-light text-[#EAE1D5]/70 flex flex-col items-center md:items-start">
              <div className="space-y-1">
                <p className="flex items-center space-x-2">
                  ...
                  <a href={`tel:${settings?.supportPhone || "+919999999999"}`} className="hover:text-white transition-colors tracking-widest">
                    {settings?.supportPhone || "+91 99999 99999"}
                  </a>
                </p>
              </div>
              <div className="space-y-3">
                <div className="flex items-center space-x-4">
                  <a href={`mailto:${settings?.supportEmail || "support@example.com"}`} className="hover:text-white transition-colors border border-[#EAE1D5]/30 rounded-full p-2" title="Email Us"><Mail size={16} /></a>
                  <a href={`tel:${settings?.supportPhone || "+919999999999"}`} className="hover:text-white transition-colors border border-[#EAE1D5]/30 rounded-full p-2" title="Call Us"><Phone size={16} /></a>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col space-y-6">
          <div className="flex items-center space-x-3 border-b border-[#EAE1D5]/10 pb-6">
            ...
            <LinkNext href="https://www.youtube.com" target="_blank" className="w-8 h-8 rounded-full border border-[#EAE1D5]/30 flex items-center justify-center hover:bg-white/10 transition-colors text-[#EAE1D5]" title="YouTube">
            ...
            <LinkNext href="https://pinterest.com" target="_blank" className="w-8 h-8 rounded-full border border-[#EAE1D5]/30 flex items-center justify-center hover:bg-white/10 transition-colors text-[#EAE1D5]" title="Pinterest">
            ...
            <LinkNext href="https://x.com" target="_blank" className="w-8 h-8 rounded-full border border-[#EAE1D5]/30 flex items-center justify-center hover:bg-white/10 transition-colors text-[#EAE1D5]" title="X (Twitter)">
              <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg>
            </LinkNext>
            <LinkNext href={settings?.socialLinks?.facebook || "https://facebook.com"} target="_blank" className="w-8 h-8 rounded-full border border-[#EAE1D5]/30 flex items-center justify-center hover:bg-white/10 transition-colors text-[#EAE1D5]" title="Facebook">
              <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
            </LinkNext>
            <LinkNext href={settings?.socialLinks?.instagram || "https://instagram.com"} target="_blank" className="w-8 h-8 rounded-full border border-[#EAE1D5]/30 flex items-center justify-center hover:bg-white/10 transition-colors text-[#EAE1D5]" title="Instagram">
            ...
            <LinkNext href="https://linkedin.com" target="_blank" className="w-8 h-8 rounded-full border border-[#EAE1D5]/30 flex items-center justify-center hover:bg-white/10 transition-colors text-[#EAE1D5]" title="LinkedIn">
              <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" /></svg>
            </LinkNext>
            <LinkNext href="https://tumblr.com" target="_blank" className="w-8 h-8 rounded-full border border-[#EAE1D5]/30 flex items-center justify-center hover:bg-white/10 transition-colors text-[#EAE1D5]" title="Tumblr">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center text-[11px] font-light text-[#EAE1D5]/50 space-y-4 md:space-y-0">
            <span>© 2026 Luxury Jewelry Commerce Starter. All Rights Reserved.</span>
